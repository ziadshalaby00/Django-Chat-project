...
/**
 * Patches for django-unfold's app.js
 * ------------------------------------------------------------
 * Load this file AFTER unfold/js/app.js (see UNFOLD["SCRIPTS"]
 * in settings.py). Both functions below simply get re-declared
 * in the global scope, so they override unfold's originals.
 *
 * Fix 1: sidebar collapsing on every full-page navigation
 * ---------------------------------------------------------
 * The current app.js ties sidebarOpen() to window.innerWidth:
 *
 *     sidebarOpen() {
 ...
 *     }
 *
 * Since Django admin does a full page reload on every link
 * click, Alpine re-runs theme() from scratch each time. If the
 * browser's usable width is <= 1280px (very common on laptop
 * screens, with devtools open, or with browser zoom), the
 * sidebar is forced closed on EVERY navigation, no matter what
 * was saved in localStorage — which looks like "it keeps
 * closing itself".
 *
 * This version removes the width-based override and lets
 * localStorage be the single source of truth, matching how the
 * older (0.96.0) app.js behaved before sidebarOpen/sidebarToggle
 * were introduced.
 */
	...
	return {
		...
		init() {
			this.$watch("filterOpen", (value) => {
				if (typeof isFilterModalOpen === "function" && isFilterModalOpen()) {
...
/**
 * Fix 2: crispyFormsetToggleAdd crashing the page
 * ---------------------------------------------------------
 * crispyFormset() is brand new (not present in 0.96.0). It runs
 * on every window "load" event and, for every element with the
 * class .crispy-formset-add, does:
 *
 *     document.querySelector(`#${formsetId} input[name*="TOTAL_FORMS"]`).value
 *
 * If that selector doesn't match anything on the current page
 * (wrong/missing formsetId, management-form inputs not
 * rendered, etc.), this throws:
 *   "Cannot read properties of null (reading 'value')"
 * which aborts the rest of that forEach loop AND the
 * document-level "formset:added" listener registration that
 * comes after it in crispyFormset() — breaking formset add/
 * delete behaviour for the whole page.
 *
 * This version just skips a malformed/missing formset instead
 * of throwing, and warns in the console so you can find and fix
 * the actual template/admin config issue.
 */
function crispyFormsetToggleAdd(formsetId) {
	const totalFormsEl = document.querySelector(
		`#${formsetId} input[name*="TOTAL_FORMS"]`,
	);
	const maxFormsEl = document.querySelector(
		`#${formsetId} input[name*="MAX_NUM_FORMS"]`,
	);

	if (!totalFormsEl || !maxFormsEl) {
		console.warn(
			`[unfold-patches] crispyFormsetToggleAdd: management inputs for formset "${formsetId}" were not found — skipping.`,
		);
		return;
	}

	const totalForms = parseInt(totalFormsEl.value, 10);
	const maxNumForms = parseInt(maxFormsEl.value, 10);

	const wrapper = document.getElementById(`${formsetId}-add-row-wrapper`);
	if (!wrapper) {
		return;
	}
